...
```bash
python3 -m uvicorn main:app --host 0.0.0.0 --port 8000 >/tmp/monopoly.log 2>&1 &
```
...
curl -s http://localhost:8000/api/robot/health          # stub|live
curl -s http://localhost:8000/api/game/state | python3 -m json.tool
curl -s http://localhost:8000/api/game/winner
```

WebSocket 이벤트 구독 (예: websocat 사용):

```bash
websocat ws://localhost:8000/api/stream/game
```
...
| `Address already in use` (port 8000) | 이전 프로세스 살아있음. `pkill -f 'uvicorn main:app'` 또는 `MONOPOLY_PORT=8001` 로 다른 포트 사용. |
